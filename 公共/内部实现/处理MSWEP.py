...
输入：
- 数据/原始气象/降水/MSWEP/*.nc  (从Google Drive下载的MSWEP V2.8 daily文件)
  文件命名格式: YYYYDDD.nc (年+儒略日)

输出：
- 数据/原始气象/降水/MSWEP_日尺度/P_YYYY.MM.DD.tif

使用说明：
1. 从Google Drive下载 MSWEP V2.8 -> Past -> Daily 文件夹中的NC文件
2. 只需下载 2006-2020 年的文件 (2006001.nc ~ 2020366.nc)
3. 将文件放入 数据/原始气象/降水/MSWEP/
4. 运行此脚本
"""
...
MSWEP_INPUT_DIR = _prefer_existing_path(os.path.join(PREC_ROOT, "MSWEP"), os.path.join(PREC_ROOT, "mswep"))
PREC_OUTPUT_DIR = _prefer_existing_path(os.path.join(PREC_ROOT, "MSWEP_日尺度"), os.path.join(PREC_ROOT, "daily"))

# 当前流域范围 [West, South, East, North]
TUOTUOHE_BBOX = {
    'lon_min': 90.5,
    'lon_max': 93.5,
    'lat_min': 33.0,
...
def process_mswep_file(nc_file, output_dir):
    """处理单个MSWEP NC文件，提取默认流域区域并保存为GeoTIFF"""
