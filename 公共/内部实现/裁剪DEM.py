...
BASIN_SHP = os.path.join(GIS_DIR, "basin.shp")

# 输出
OUTPUT_DEM = os.path.join(GIS_DIR, "dem_1km.tif")
...
def clip_dem_to_basin():
    ...
    try:
        area_km2 = basin.to_crs(epsg=6933).geometry.area.sum() / 1e6
    except Exception:
        area_km2 = basin.geometry.area.sum() / 1e6
    print(f"   流域面积: {area_km2:.2f} km2")
    ...
    with rasterio.open(OUTPUT_DEM) as dem:
        data = dem.read(1).astype("float64")
        ...
        print(f"   CRS: {dem.crs}")
        if valid.size > 0:
            print(f"   高程范围: {np.nanmin(valid):.1f} ~ {np.nanmax(valid):.1f} m")
            print(f"   平均高程: {np.nanmean(valid):.1f} m")
            print(f"   有效像元: {valid.size}")
        else:
            print("   [WARN] 裁剪后无有效像元，请检查流域边界与DEM是否重叠。")
...
def visualize_dem():
    ...
    ax.set_title('Tuotuohe Basin DEM')
