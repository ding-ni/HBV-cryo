...
输入：
- 数据/地理数据/dem_1km.tif                    参考DEM
- 数据/地理数据/basin.shp             流域边界
- 数据/原始气象/降水/MSWEP_日尺度/*.tif      原始降水
- 数据/原始气象/气温/日尺度/*.tif        原始温度
- 数据/原始气象/蒸散发/日尺度/*.tif        原始蒸散发

输出：
- 数据/模型输入/降水_MSWEP/*.tif         对齐并裁剪的降水
- 数据/模型输入/气温/*.tif         对齐并裁剪的温度
- 数据/模型输入/蒸散发/*.tif         对齐并裁剪的蒸散发

修复日志：
- 2026-01-09: 修复文件名按字典序排序的bug，改为按日期排序
- 2026-01-09: 整合流域掩膜步骤，直接输出到模型输入目录
"""
...
INPUT_DIRS = {
    "prec": _prefer_existing_path(
        os.path.join(RAW_ROOT, "降水", "MSWEP_日尺度"),
        os.path.join(RAW_ROOT, "precipitation", "daily"),
    ),
    "temp": _prefer_existing_path(
        os.path.join(RAW_ROOT, "气温", "日尺度"),
        os.path.join(RAW_ROOT, "temperature", "daily"),
    ),
    "evap": _prefer_existing_path(
        os.path.join(RAW_ROOT, "蒸散发", "日尺度"),
        os.path.join(RAW_ROOT, "evaporation", "daily"),
    ),
}
