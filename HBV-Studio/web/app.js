...
function toWizardInputTimeValue(value, hourly) {
  return window.HBVStudioDataPrepView.toWizardInputTimeValue(value, hourly);
}

function fromWizardInputTimeValue(value, hourly) {
  return window.HBVStudioDataPrepView.fromWizardInputTimeValue(value, hourly);
}
...
function syncCustomMeteoImportInputs(force = false) {
  const copyState = window.HBVStudioDataPrepView.customMeteoImportCopyState({
    sources: getWizardMeteoSources(),
    registeredDirs: {
      prec: $("#wz-custom-prec-dir")?.value || "",
      temp: $("#wz-custom-temp-dir")?.value || "",
      pet: $("#wz-custom-pet-dir")?.value || "",
    },
    importDirs: {
      prec: $("#wz-import-prec-dir")?.value || "",
...
async function saveCurrentWizardStep() {
  ...
  try {
    const request = window.HBVStudioDataPrepView.wizardSaveStepRequestState({
      workspacePath: state.wizardWorkspacePath,
      step,
      data,
    });
...
function updateGisMode() {
  const panel = window.HBVStudioDataPrepView.gisModePanelState(getSelectedRadio("wz-gis-mode"));
  $("#wz-gis-auto-panel").classList.toggle("hidden", !panel.panels.autoVisible);
  $("#wz-gis-import-panel").classList.toggle("hidden", !panel.panels.importVisible);
}
...
async function refreshEra5ApiStatus({ force = false } = {}) {
  ...
  renderEra5ApiPanel();
  try {
    const payload = await apiGet(window.HBVStudioDataPrepView.cdsApiStatusQueryState().statusPath);
...
async function refreshCurrentWorkspaceWorkflow() {
  ...
  const query = window.HBVStudioDataPrepView.workspaceReadinessQueryState({
    configPath: state.wizardWorkspacePath,
    precipSource: getTaskRuntimePrecipSource(),
  });
  if (!query.ready) return null;
  const p = await apiGet(query.completenessPath);
...
async function refreshCurrentWorkspaceAdvice() {
  ...
  if (!query.ready) return null;
  const p = await apiGet(query.advicePath);
...
async function loadRuns() {
  const p = await apiGet(window.HBVStudioResultsView.runListQueryState().runsPath);
  ...
  Object.assign(state, window.HBVStudioResultsView.runListDataState(p.data).statePatch);
...
async function loadRun(path) {
  const query = window.HBVStudioResultsView.runDetailQueryState({ path });
  const targetPath = query.runPath;
  const previousSelected = currentSelectedRunPath();
  const previousRunData = state.currentRun;
  const requestToken = runDetailRequestGuard.next();
  if (!query.ready) {
    clearRunDetail(query.message);
    return;
  }
  ...
  renderRunList();
  try {
    const p = await apiGet(query.detailPath);
