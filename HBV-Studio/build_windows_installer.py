...
def output_label_suffix(output_label: str) -> str:
    label = re.sub(r'[<>:"/\\|?*\x00-\x1f]+', "_", str(output_label or "").strip())
    ...
    *,
    ...
    manifest = {
        ...
        "package_variant": {
            ...
            "license_enforced": license_mode != "unlimited",
        },
    ...
    output = OUTPUT_DIR / f"BUILD_MANIFEST_{version}{output_label_suffix(output_label)}.json"
...
def configure_staged_license(bundle_dir: Path, license_mode: str) -> Path:
    normalized = str(license_mode or "project").strip().lower()
    if normalized not in {"project", "unlimited"}:
        raise ValueError(f"Unsupported license mode: {license_mode}")
    variant_file = bundle_dir / "HBV-Studio" / "services" / "package_variant.py"
    variant_file.parent.mkdir(parents=True, exist_ok=True)
    variant_file.write_text(
        "# -*- coding: utf-8 -*-\n"
        '"""Generated by build_windows_installer.py; do not edit in staged output."""\n\n'
        f'LICENSE_MODE = "{normalized}"\n',
        encoding="utf-8",
    )
    return variant_file
...
def write_inno_setup_script(bundle_dir: Path, version: str, *, output_label: str = "") -> Path:
    spec_dir = prepare_clean_dir(INNO_BUILD_ROOT / "spec")
    output_dir = OUTPUT_DIR.resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    installer_name = f"HBVStudio_Setup_{version}{output_label_suffix(output_label)}"
...
def main() -> None:
    ...
    parser.add_argument(
        "--license-mode",
        choices=("project", "unlimited"),
        default="project",
        help="project=固定到期日外发版；unlimited=软件所有者本地无期限版。",
    )
    parser.add_argument(
        "--output-label",
        default="",
        help="附加到安装包和构建清单文件名的标签，例如 外发授权版。",
    )
    ...
    configure_staged_license(bundle_dir, args.license_mode)
    compile_and_strip_source(bundle_dir)

    script_path = write_inno_setup_script(bundle_dir, args.version, output_label=args.output_label)
