...
def _geo_empty_layer(
    ...
    return {
        ...
        "status": status,
        "message": message,
        "path": str(path.resolve(strict=False)) if path is not None else "",
        "display_path": to_display_path(path.resolve(strict=False)) if path is not None else "",
        "bounds": None,
        "rings": [],
        "metrics": {},
    }
...
def _vector_geo_layer(layer_id: str, label: str, path: Path | None) -> dict[str, Any]:
    if path is None:
        return _geo_empty_layer(layer_id, label, "vector", None, "missing", "未配置")
    if not path.exists():
        return _geo_empty_layer(layer_id, label, "vector", path, "missing", "文件不存在")
    try:
        import geopandas as gpd

        gdf = gpd.read_file(path)
        if gdf.empty:
            return _geo_empty_layer(layer_id, label, "vector", path, "missing", "文件为空")
        if gdf.crs:
        ...
        try:
            if path.stat().st_size <= 8 * 1024 * 1024:
                ...
                rings = _geometry_to_geo_rings(geometry, bounds)
        except Exception:
            rings = []
        if not rings:
        ...
        return {
            ...
            "display_path": to_display_path(path.resolve(strict=False)),
            "bounds": bounds,
            "rings": rings,
            "metrics": {"feature_count": int(len(gdf))},
        }
    except Exception as exc:
        return _geo_empty_layer(layer_id, label, "vector", path, "error", f"读取失败：{exc}")


def _raster_geo_layer(layer_id: str, label: str, path: Path | None) -> dict[str, Any]:
    if path is None:
        return _geo_empty_layer(layer_id, label, "raster", None, "missing", "未配置")
    if not path.exists():
        return _geo_empty_layer(layer_id, label, "raster", path, "missing", "文件不存在")
    try:
        import numpy as np
        import rasterio
...
def workspace_geo_overview(config_path_raw: str) -> dict[str, Any]:
    ...
    configured_dem_path = _resolve_config_related_path(config, config.get("DEM_tif"))
    workspace_dem = _workspace_dem_path(gis_dir, prefer=_configured_dem_kind(config))
    ...
    glacier_vector = _resolve_config_related_path(config, config.get("冰川边界_shp")) if raw_glacier else None
    glacier_path = glacier_raster or glacier_vector

    layers = [
        _vector_geo_layer("basin", "流域边界", basin_path),
        _raster_geo_layer("dem", "DEM", dem_path),
        _raster_geo_layer("elevation_zone", "高程分区", zone_path),
        (
            _raster_geo_layer("glacier", "冰川", glacier_path)
            if glacier_path and glacier_path.suffix.lower() in {".tif", ".tiff"}
            else _vector_geo_layer("glacier", "冰川", glacier_path)
        ),
    ]
    ...
    focus_bounds = vector_focus or _merge_geo_bounds([layer.get("bounds") for layer in ok_layers if layer.get("kind") == "vector"]) or bounds
    return {
